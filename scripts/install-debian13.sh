...
UPLOAD_RATE_LIMIT_MAX="${UPLOAD_RATE_LIMIT_MAX:-6}"
DEEPAUDIO_ACCESS_TOKEN="${DEEPAUDIO_ACCESS_TOKEN:-}"
PORT="${PORT:-3001}"
DOMAIN="${DOMAIN:-}"
...
check_platform() {
  ...
  [[ "${UPLOAD_RATE_LIMIT_WINDOW_MS}" =~ ^[0-9]+$ ]] &&
    ((UPLOAD_RATE_LIMIT_WINDOW_MS > 0)) ||
    fatal "UPLOAD_RATE_LIMIT_WINDOW_MS deve ser maior que zero."
  [[ "${UPLOAD_RATE_LIMIT_MAX}" =~ ^[0-9]+$ ]] ||
    fatal "UPLOAD_RATE_LIMIT_MAX deve ser um numero maior ou igual a zero."
